    ...
    public class UserBusiness : Bussiness<User>
    {
        ...
        public User getOneByUserPass(string user, string pass)
        {
            List<User> result = base.select(
                $"t.{idColumn}, t.{String.Join(" ,t.", columns)}, r.Descripcion as RoleDescription, eu.Descripcion as EstadoDescription", $" INNER JOIN Roles r ON r.Id=t.RolId INNER JOIN Estados_Usuarios eu ON eu.Id = t.EstadoId WHERE (t.Username='{user}' OR t.Email='{user}') AND t.Password='{pass}'");
